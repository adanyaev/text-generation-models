...
class Execution:
	...
	def __init__(self, args):
		self.file = 'pre_meduza1_signs.txt'
	...
	def train(self, args):
		...
		for epoch in range(self.num_epochs):
			...
			print("Epoch: %d,  loss: %.5f " % (epoch, min_loss))
			torch.save(model.state_dict(), 'weights/textGenerator_meduza_model.pt')
	...
	def generator(model, sequences, idx_to_char, n_chars):
      ...
      # number of characters
		for i in range(n_chars):
			...
			pattern = pattern.to(torch.device('cuda'))
			...
			pattern = pattern.to(torch.device('cpu'))
...
if __name__ == '__main__':
	...
	if args.load_model == True:
		...
		# Load weights
		model.load_state_dict(torch.load('weights/textGenerator_meduza_model.pt'))
		
		# Text generator
		execution.generator(model, sequences, idx_to_char, 2000)
