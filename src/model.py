...
class TextGenerator(nn.ModuleList):
	...
	def forward(self, x):
		...
		hs_forward = torch.zeros(x.size(0), self.hidden_dim, device=torch.device('cuda'))
		cs_forward = torch.zeros(x.size(0), self.hidden_dim, device=torch.device('cuda'))
		hs_backward = torch.zeros(x.size(0), self.hidden_dim, device=torch.device('cuda'))
		cs_backward = torch.zeros(x.size(0), self.hidden_dim, device=torch.device('cuda'))
		
		# LSTM
		# hs = [batch_size x (hidden_size * 2)]
		# cs = [batch_size x (hidden_size * 2)]
		hs_lstm = torch.zeros(x.size(0), self.hidden_dim * 2, device=torch.device('cuda'))
		cs_lstm = torch.zeros(x.size(0), self.hidden_dim * 2, device=torch.device('cuda'))
